...
function renderInvoiceCard() {
  ...
  document.getElementById('btnSendInvoice').addEventListener('click', async () => {
    ...
    const text    = `Please find the invoice for ${co}, ${mo} 2026 attached.\n\nGenerated by Claude Close.`;
    ...
    // Fallback for browsers without file-share support: download + open email client
    const url = URL.createObjectURL(blob);
    const a   = document.createElement('a');
    a.href = url; a.download = filename; a.click();
    URL.revokeObjectURL(url);
    setTimeout(() => {
      window.location.href = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(text + '\n\n(PDF downloaded — please attach before sending)')}`;
    }, 500);
  });
}
