...
import { downloadInvoicePDF, getInvoiceBlob } from './src/invoice.js';
import {
...
function renderInvoiceCard() {
  ...
  document.getElementById('btnSendInvoice').addEventListener('click', async () => {
    const { blob, filename } = getInvoiceBlob(co, mo, projects);
    const subject = `Invoice: ${co} — ${mo} 2026`;
    const body    = `Please find the invoice for ${co}, ${mo} 2026 attached.\n\nGenerated by Claude Close.`;

    // Build an .eml file with the PDF embedded — opens in Outlook/Apple Mail/etc.
    // as a ready-to-send draft with the attachment already present.
    ...
    const a       = document.createElement('a');
    a.href = url; a.download = filename.replace('.pdf', '.eml'); a.click();
    URL.revokeObjectURL(url);
  });
}
